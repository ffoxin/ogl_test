...
static TCHAR szWindowClass[]    = _T("win32app"); // window class name
static TCHAR szTitle[]          = _T("Win32 Guided Tour Application"); // title bar string
...
bool    active                  = TRUE; // window isActive flag
...
int DrawGLScene(GLvoid)
{
    ...
    glTranslatef(-1.5f,0.0f,-6.0f);     // move view

    glRotatef(rtri,0.0f,1.0f,0.0f);     // rotate

    glBegin(GL_TRIANGLES);              // figure 1
    ...
    glLoadIdentity();
    glTranslatef(1.5f,0.0f,-7.0f);      // move view

    glRotatef(rquad,1.0f,1.0f,1.0f);    // rotate

    glBegin(GL_QUADS);                  // figure 2
    ...
    rtri+=0.2f;                         // rotate figure 1
    rquad-=0.15f;                       // rotate figure 2
